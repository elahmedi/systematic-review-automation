...
def analyse(
    ...
    azure_endpoint: Optional[str] = typer.Option(
        ...
        help="Azure OpenAI endpoint URL. If set, Azure OpenAI will be used.",
    ),
    azure_api_version: Optional[str] = typer.Option(
        None,
        envvar="AZURE_OPENAI_API_VERSION",
        help="Azure OpenAI API version (default: 2024-08-06)",
    ),
) -> Optional[Framework]:
    """
    ...
    if manuscript_path.is_dir():
        ...
        error_log_path = manuscript_path / "error_log.txt"
        failed_count = 0
        ...
        for idx, pdf_path in enumerate(pdf_files, 1):
            if verbose:
                typer.echo(f"[{idx}/{len(pdf_files)}] Analysing {pdf_path.name}")
            try:
                framework = analyse(
                ...
                failed_count += 1
                error_msg = f"[FAILED] {pdf_path.name}: {type(e).__name__}: {e}\n"
        ...
        if failed_count > 0:
            typer.echo(
                typer.style(
                    f"\n⚠️  {failed_count} file(s) failed. See {error_log_path}",
