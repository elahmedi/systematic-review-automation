...
def _create_client(
    ...
    If Azure settings are provided (either via parameters or environment),
    creates an AzureOpenAI client. Otherwise, creates a standard OpenAI client.

    Parameters
    ----------
    api_key : Optional[str]
        API key for authentication. For Azure, this should be the Azure API key.
        If not provided, will use settings from environment.
    azure_endpoint : Optional[str]
        Azure OpenAI endpoint URL. If provided, Azure client will be used.
    azure_api_version : Optional[str]
        Azure OpenAI API version. Defaults to settings value if not provided.
    ...
    use_azure = azure_endpoint or settings.use_azure

    if use_azure:
        # Use Azure OpenAI
        endpoint = azure_endpoint or settings.azure_openai_endpoint
        key = api_key or settings.azure_openai_api_key
        version = azure_api_version or settings.azure_openai_api_version

        if not endpoint:
            raise ValueError(
                "Azure OpenAI endpoint is required. "
                "Set AZURE_OPENAI_ENDPOINT environment variable or provide azure_endpoint parameter."
            )
        if not key:
            raise ValueError(
...
def run_framework(
    ...
    Supports both standard OpenAI and Azure OpenAI services. Azure OpenAI is
    automatically used when Azure settings are provided via parameters or
    ...
    model : str, default=settings.fast_ai_model
        The OpenAI model identifier to use for assessment. For Azure OpenAI,
        this should be your deployment name. Different models may provide
        varying levels of analysis depth and accuracy.
    ...
    api_key : Optional[str], default=None
        API key to use for OpenAI/Azure OpenAI calls. If ``None``, the appropriate
        key from environment variables will be used (OPENAI_API_KEY or
        AZURE_OPENAI_API_KEY).
    azure_endpoint : Optional[str], default=None
        Azure OpenAI endpoint URL. If provided (or set via AZURE_OPENAI_ENDPOINT
        environment variable), Azure OpenAI will be used instead of standard OpenAI.
        Example: "https://your-resource.openai.azure.com/"
    azure_api_version : Optional[str], default=None
        Azure OpenAI API version. Defaults to "2024-08-06" if not specified.
        Only used when Azure OpenAI is enabled.
